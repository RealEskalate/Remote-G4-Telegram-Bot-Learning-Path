...
async def start_handler(message: types.Message):
    await message.answer(
        "Hello Welcome to The Hermetic Order of the blue rose",
...
async def text_message_handler(message: types.Message, state: FSMContext):
    ...
    if current_state == "waiting_for_phone":
        ...
        if matched:
            ...
            await message.answer(
                f"Thank you for sharing your contact, The Hermetic Order of the blue rose will contact you soon. Your phone number is {phone_number}"
            )

            await state.clear()
        else:
            await message.answer(
                "Please share a valid phone number. Try again."
            )
