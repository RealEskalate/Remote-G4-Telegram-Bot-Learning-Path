...
from database.services.user_services import get_user_by_id, create_user, delete_user
...
async def handle_refresh(message: types.Message, state: FSMContext):
    ...
    # 2103092364
    # 2103092364
    status = await get_user_by_id(str(userID))
    # print("->>>>",status)
    if not status:
...
async def form_phone(message: Message, state: FSMContext):
    ...
    print(name) 
    await message.answer(f"alright {name['first_name']}, your occupation", reply_markup=keyboard.cancel_inline_keyboard)


@registration_router.message(Form.occupation)
...
async def show_profile_handler(message: types.Message, state: FSMContext):
    ...
    if curr_state: 
        ...
        if not state_data :
            ...
            data = await get_user_by_id(state_data['user_id'])
...
async def delete_profile_handler(message: types.Message, state: FSMContext):
    await message.delete()
    ...
    if curr_state: 
        ...
        if not state_data :
            ...
            if data: 
                ...
                delete = await delete_user(state_data['user_id'])
                if delete:
                    await state.clear()
